...
```{r}
head(df_seeds, 20)
nrow(df_seeds) # número filas
ncol(df_seeds) # número columnas
str(df_seeds)
#summary(df_seeds)
```
...
df_seeds |> group_by(tipo) |> summarise_at(vars(area), list(area_media = mean))
...
**Respuesta**: 'Estamos seleccionando varias columnas para realizar una clusterización por kmeans (agrupación de observaciones en k clústeres). El resultado son 3 clústeres que comparamos con el tipo original de semilla del dataframe'
